...
names(gmr_lad_by_day_sf)[1] <- 'id_join'
names(gmr_lad_by_day_sf)[2] <- 'cntry_cod'
...
measure.vars.melt <- names(gmr_lad_by_day)[11:16]
...
gmr_lad_by_day_charts$sb_rg_1 <- NULL;gc()
gmr_lad_by_day_charts$sb_rg_2 <- NULL;gc()
...
gmr.lad.lut <- 
   merge(gmr.lad.list,lad.code.list,by.x='lad19cd',
         by.y='lad.codes')
# Creating the LAD folders
lapply(1:length(gmr.lad.lut$lad19cd),function(l){
   #l <- 294
   setwd(cDIR)
   lad.name <- gmr.lad.lut[l,]$lad19nm
   lad.code <- gmr.lad.lut[l,]$lad19cd
   if(!dir.exists(paste0(lad.code,' - ',lad.name))) {dir.create(paste0(lad.code,' - ',lad.name))}
   setwd(paste0(lad.code,' - ',lad.name))
   ...
   file.copy(paste0(cdataF,'covid19importantdates.png'),
                    '../img/',overwrite = T)
   file.copy(paste0(cDIR,'/html.report.Rmd'),
             '../',overwrite = T)
   file.rename('../html.report.Rmd',paste0('../',lad.code,' - ',lad.name,'-mobility-report.Rmd'))
   file.copy(paste0(cDIR,'/1.charts.preparation.R'),
             '../',overwrite = T)
   # Loading the UK Bank Holidays
   ...
   gmr.lad.lut$sub_region_1 <- sub_region_1_name
   fwrite(gmr.lad.lut[l,],'gmr.lad.lut.csv')
   lad2019.chart <- 
